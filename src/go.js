...
const round = (val) =>
  val === null || val === undefined ? null : Math.round(Number(val));

export function logDatabaseStatus() {
  const db = new DBHelper();
  try {
    const count = db.getRowCount();
    console.log(`Database check: ${count} rows found in 'coordinates' table.`);
  } catch (error) {
    console.error("Failed to perform database check:", error);
  } finally {
    db.close();
  }
}
...
export async function syncHistory() {
  ...
  try {
    ...
    for (const entry of allEvents) {
      ...
      if (config.sensors[entityId]) {
        ...
        if (colName === "is_charging") {
          ...
          colName.includes("battery_level") ||
          colName.includes("signal_strength") ||
          colName.includes("car_battery")
        ) {
          currentStates[colName] = isNaN(Number(val))
            ? null
            : Math.round(Number(val));
        } else {
          // For text fields (wifi_ssid, network_type, battery_state, car_fuel_type)
          currentStates[colName] =
            val === "unavailable" || val === "unknown" ? null : val;
        }
      }
    ...
    console.log(
      `Success! Processed ${allEvents.length} events. Saved ${savedCount} records.`,
    );
  } catch (error) {
